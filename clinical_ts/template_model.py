...
from .data.time_series_dataset import tsdata_collate_fn, TimeSeriesDataset, TimeSeriesDatasetConfig, ConcatTimeSeriesDataset
...
class BaseModel(lp.LightningModule):
    ...
    def train_dataloader(self):
        return DataLoader(self.train_dataset, collate_fn=tsdata_collate_fn, batch_size=self.hparams.base.batch_size, num_workers=4, shuffle=True, drop_last = True)
        
    def val_dataloader(self):
        return DataLoader(self.val_dataset, collate_fn=tsdata_collate_fn, batch_size=self.hparams.base.batch_size, num_workers=self.hparams.trainer.num_workers)
    
    def test_dataloader(self):
        if(self.hparams.loss.loss_type=="supervised"):#multiple val dataloaders
            return [DataLoader(self.test_dataset_val, collate_fn=tsdata_collate_fn, batch_size=self.hparams.base.batch_size, num_workers=self.hparams.trainer.num_workers),DataLoader(self.test_dataset_test, collate_fn=tsdata_collate_fn, batch_size=self.hparams.base.batch_size, num_workers=self.hparams.trainer.num_workers)]
        else:
            return DataLoader(self.val_dataset, collate_fn=tsdata_collate_fn, batch_size=self.hparams.base.batch_size, num_workers=self.hparams.trainer.num_workers)
