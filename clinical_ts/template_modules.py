...
class BaseConfig:
    #######################################################################################################
    only_final_test: bool = False # tw: test only on the final test set (i.e. no validation set in testing)
    #########################################################################################################
    #optimizer
...
class TimeSeriesEncoder(EncoderBase):
    ...
    def __init__(self, hparams_seqenc, hparams_input_shape, static_stats_train, target_dim=None, has_ema=True):
        super().__init__(hparams_seqenc, hparams_input_shape, static_stats_train)
        # save basic vars
    ...
    def forward(self, **kwargs): 
        ...
        if(self.loss is None): #standard supervised forward
            ...
            result_wtz = {k:v for k,v in seq_pred.items() if (k not in kwargs.keys() or not kwargs[k] is v)} #bs,seq'',feat
            del seq_w_static_enc_res, seq_w_static_enc, seq_enc_w_static_enc_res, seq_enc_w_static_enc, seq_pred_res, seq_pred
