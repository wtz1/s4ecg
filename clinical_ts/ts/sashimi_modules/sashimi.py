#from https://github.com/HazyResearch/state-spaces/blob/main/sashimi/sashimi.py
"""
SaShiMi backbone.
...
from clinical_ts.s4original import S4 as S4original
from clinical_ts.s4original import LinearActivation
...
class Sashimi(nn.Module):
    def __init__(
        ...
        Args:
            ...
            NEW d_state: H in S4 layers
            NEW d_input: input dimension (None to skip linear encoder)
            NEW d_output: output dimension (None to skip linear decoder)
            NEW transposed_input: (B, d_input/d_output, ts) instead of (B, ts, d_input/d_output) if set to true
            NEW backbone: s4original, s4new, s4d
        ...
        self.encoder = nn.Conv1d(d_input,d_model,kernel_size=1) if d_input is not None else nn.Identity()
        self.decoder = nn.Conv1d(d_model,d_output,kernel_size=1) if d_output is not None else nn.Identity()
