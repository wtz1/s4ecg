...
class VGGishClassifier(nn.Module):
    def __init__(self,n_classes, pretrained=True):
        super().__init__()
        self.encoder = VGGish(pretrained=pretrained)
        self.classifier = nn.Linear(128, n_classes)

    def forward(self,x):
        #assuming input is bs, 1, mel, ts- vggish expects bs, 1, ts, mel
        return self.classifier(self.encoder(torch.unsqueeze(torch.transpose(x,1,2),dim=1)))


class VGGish(VGG):
    def __init__(self, urls=model_urls, pretrained=True, postprocess=False, progress=True):
