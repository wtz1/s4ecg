...
from .components import OurModule
from .mlp import init_mlp
from .ssm import init_ssm
from .ssm.preprocess import init_preprocess_ssm as init_pre
...
class Decoder(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.closed_block = self.init_closed_block(config)#modified
        self.open_blocks = self.init_open_blocks(config)
        
    def init_closed_block(self, config):#modified
        return ClosedLoopBlock(**config['blocks'][0])
    
    def init_open_blocks(self, config):#modified
    ...
    def forward(self, x):
        y, us = self.closed_block(x) # y, (u_next, u)
        return self.open_blocks(y),us if self.open_blocks is not None else y,us #modified  
