...
from clinical_ts.ts.basic_conv1d_modules.basic_conv1d import create_head1d
...
class XResNet1d(nn.Sequential):
    def __init__(self, block, expansion, layers, input_channels=3, num_classes=1000, stem_szs=(32,32,64), input_size=1000, heads=4, mhsa=False, kernel_size=5,kernel_size_stem=5,
        ...
        if(input_size is not None):
            self.output_size = math.floor((input_size-1)/2+1) #strided conv and MaxPool1d
            self.output_size = math.floor((self.output_size-1)/2+1)
        ...
        if(num_classes is None):
            ...
            self.output_features = block_szs[-1]*expansion
        else:
    ...
    def _make_layer(self, ni, nf, blocks, stride, kernel_size, heads=4, mhsa=False, **kwargs):
        input_size0 = self.output_size
        input_size1 = math.floor((self.output_size-1)/stride+1) if self.output_size is not None else None
        self.output_size = input_size1
    ...
    def get_output_shape(self):
        return [self.output_features, self.output_size]
